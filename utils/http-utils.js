 ...
 * 不跳转自动登录
 */
const postNoAutoLogin = (request,interfaceName,hideLoading)=>{
	return new Promise((resolve,inject)=>{
		commonFuntcion.checkStorgeHasUser(false).then(token=>{
			realPost(request,interfaceName,token,hideLoading).then((res)=>{
				resolve(res);
...
const post = (request,interfaceName,hideLoading,hideError)=>{
	return new Promise((resolve,inject)=>{
			realPost(request,interfaceName,hideLoading,hideError).then((res)=>{
				resolve(res);
			}).catch((reason)=>{inject(reason)})
	})
}
...
const get = (request,interfaceName,hideLoading)=>{
	return new Promise((resolve,inject)=>{
			realGet(request,interfaceName,hideLoading).then((res)=>{
				resolve(res);
			}).catch((reason)=>{inject(reason)})
	})
}
 ...
 * 跳转自动登录form-data请求
 */
const postParams = (request,interfaceName,hideLoading)=>{
	return new Promise((resolve,inject)=>{
		commonFuntcion.checkStorgeHasUser(true).then(token=>{
			realPostParams(request,interfaceName,token,hideLoading).then((res)=>{
				resolve(res);
			}).catch((reason)=>{inject(reason)})
		}).catch(()=>{inject('尚未登录!')})
	})
